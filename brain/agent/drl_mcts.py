...
class DRL_MCTS(BaseAgent, LearningBaseAgent):
    ...
    def _predict_policy_value_with_model(self, model, state: SearchState) -> Tuple[np.ndarray, float]:
        ...
        q_values, valid_mask = self._q_values(
            list(state.board), color, aux_features, legal_actions, model=model
        )
        ...
        if len(legal_q) > 0:
            scaled = legal_q / float(self.prior_temperature)
            exp_q = np.exp(scaled - np.max(scaled))
            probs = exp_q / np.sum(exp_q)
            priors[legal_indices] = probs.astype(np.float32)
            value = float(np.max(legal_q))
            value = max(-1.0, min(1.0, value))
        else:
            value = 0.0
        return priors, value

    def _create_mcts(self, model) -> PUCTMCTS:
        return PUCTMCTS(
            simulator=self.simulator,
            policy_value_fn=lambda s: self._predict_policy_value_with_model(model, s),
    ...
    def _action(self) -> Tuple[int, int]:
        ...
        mcts = self._create_mcts(self.model)
    ...
    def _self_play_episode(
        ...
        current_mcts = self._create_mcts(self.model)
        opponent_mcts = self._create_mcts(self.opponent_model) if use_past_opponent else None
        ...
        while True:
            ...
            active_model = self.model if (is_drl_turn or not use_past_opponent) else self.opponent_model

            # Log avg q_max for this state (for diagnostics)
            q_values_diag, valid_mask_diag = self._q_values(
                list(state.board), color, aux_features, legal_actions, model=active_model
            )
            legal_idx_diag = np.flatnonzero(valid_mask_diag > 0)
            if len(legal_idx_diag) > 0:
                stats.q_max_sum += float(np.max(q_values_diag[legal_idx_diag]))

            # MCTS-based action selection
            temp = 1.0 if stats.steps < self.temp_threshold else 0.0
        ...
        for i, (_, aux_t, action_idx, valid_t, _, player_t, raw_state, action_tuple) in enumerate(raw_steps):
            if use_past_opponent and player_t != drl_player_value:
                continue
