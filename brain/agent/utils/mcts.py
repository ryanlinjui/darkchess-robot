...
from brain.utils import available, get_chess_color, get_chess_pool, get_draw_limit
...
class DarkChessSimulator:
    ...
    def is_draw_state(self, state: SearchState) -> bool:
        return state.draw_steps >= self.draw_limit

    def valid_actions(self, state: SearchState) -> List[Tuple[int, int]]:
        if self.is_draw_state(state):
            return []
...
class PUCTMCTS:
    ...
    def search(
        ...
        for action_idx in valid_indices:
            ...
            if u > best_score + 1e-8:
                best_score = u
                best_action_indices = [int(action_idx)]
            elif np.isclose(u, best_score, atol=1e-8, rtol=1e-6):
                best_action_indices.append(int(action_idx))

        if len(best_action_indices) == 0:
            return 0.0
        best_action_idx = int(self.rng.choice(best_action_indices))
